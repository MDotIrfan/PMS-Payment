...
if (isset($_POST['submit'])) {
 ...
 if (!empty($bln)&&!empty($thun)) {
  ...
  $q = mysqli_query($conn, "SELECT * FROM pekerjaan WHERE MONTH(tgl_dibuat) = '$bln' and year(tgl_dibuat) = '$thun' and status ='Selesai Pembayaran'");
 } else {
  // perintah tampil semua data
  $q = mysqli_query($conn, "SELECT * FROM pekerjaan p where status ='Selesai Pembayaran'");
 }
} else {
 // perintah tampil semua data
 $q = mysqli_query($conn, "SELECT * FROM pekerjaan where status ='Selesai Pembayaran'");
}
...
<div class="col">
   ...
   <div class="card-body">
    ...
    <a href="<?php echo site_url('pm/generatelaporan/'.$bln.'/'.$thun)?>"><button type="button" name="submit" class="btn btn-success">Cetak</button></a>
   </div>
